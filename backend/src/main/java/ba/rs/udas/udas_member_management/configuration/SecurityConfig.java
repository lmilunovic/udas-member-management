        ...
        http.cors(cors -> cors.configurationSource(corsConfigurationSource()))
                ...
                .authorizeHttpRequests(
                        authorize ->
                                authorize
                                        .requestMatchers(
                                                antMatcher("/api/v1/health"), antMatcher("/api/v1"))
                                        .permitAll()
                                        .requestMatchers(antMatcher("/api/v1/auth/logout"))
                                        .permitAll()
                                        .requestMatchers(
                                                antMatcher(HttpMethod.GET, "/api/v1/members"),
                                                antMatcher(HttpMethod.GET, "/api/v1/members/**"))
                                        .hasAnyRole("READ_ONLY", "READ_WRITE", "ADMIN")
                                        .requestMatchers(
                                                antMatcher("/api/v1/members"),
                                                antMatcher("/api/v1/members/**"))
                                        .hasAnyRole("READ_WRITE", "ADMIN")
                                        .requestMatchers(antMatcher("/api/v1/users/me"))
                                        .authenticated()
                                        .requestMatchers(
                ...
                .logout(
                        logout ->
                                logout.logoutSuccessHandler(redirectLogoutSuccessHandler())
                                        .permitAll())
                .exceptionHandling(
                        ex ->
